...
static BYTE RwRasterDestroy_HookJmpCode[] = {0xFF,0x25,0x21,0xC7,0x59,0x00};
static BYTE AddAnimation_HookJmpCode[] = {0xFF,0x25,0x34,0x39,0x4D,0x00,0x90,0x90,0x90,0x90};
static BYTE BlendAnimation_HookJmpCode[] = {0xFF,0x25,0x09,0x46,0x4D,0x00,0x90};
...
static CVehicle* pEnteringVehicle;
bool NotifyEnterVehicle()
{
	if(pNetGame) {
		...
		pEnteringVehicle=pVehiclePool->GetAt(VehicleID);
		if(pEnteringVehicle->m_bDoorsLocked) return false;
		if (pEnteringVehicle->m_pVehicle->entity.nModelIndex == 570) return false;
...
NUDE TaskExitVehicle()
{
	...
	dwState = 0xFFFFFFFF;
	// dword_10151488 (bIgnoreNextExit) does nothing, removed from here
			...
			if (VehicleID != INVALID_VEHICLE_ID) {
				...
				if(pVehicle && pLocalPlayer) {
					if (pVehicle->IsATrainPart()) {
						...
						mat.pos.Y += 2.5f;
						mat.pos.X += 2.5f;
						pLocalPlayer->GetPlayerPed()->RemoveFromVehicleAndPutAt(
							mat.pos.X, mat.pos.Y, mat.pos.Z);
					} //else {
						// BUGFIX: Trigger exit vehicle notification from train also.
...
BOOL _stdcall IsFriendlyFire(PED_DAMAGE_TYPE *pDamageIssuer,PED_TYPE *pPlayer)
{
	...
	if (pIssuer != pPedPlayer && pPlayer && pPlayer->entity.vtable == 0x86C0A8)
		return 1;
...
PED_TYPE* _pAnimPed;
DWORD _dwAnimRW, _dwAninmPedRW;
DWORD _dwAnimID1, _dwAnimID2;
DWORD _dwAddAnimRetJmp = 0x4D3AAA;
DWORD dwAnimID1Saved, dwAnimID2Saved;

NUDE AddAnimation_Hook()
{
	//_asm mov edx, [esp]
	//_asm mov dwRetAddr, edx
	_asm mov edx, [esp+4]
	_asm mov _dwAnimRW, edx
	_asm mov edx, [esp+8]
	_asm mov _dwAnimID1, edx
	_asm mov edx, [esp+12]
	_asm mov _dwAnimID2, edx
	_asm pushad
	...
	_asm jmp _dwAddAnimRetJmp
}

//-----------------------------------------------------------

DWORD _dwBlendAnimDelta;
...
NUDE BlendAnimation_Hook()
{
	...
	_asm popad
	_asm sub esp, 20
	_asm mov ecx, [esp+20+4]
	_asm jmp _dwBlendAnimRetJmp
}
...
extern CObjectPool* g_pObjectPool;

// TODO: Add plate, textdraw, object processing and more...
NUDE CGame_Process_Hook()
{
	if (g_pObjectPool)
		g_pObjectPool->Process();
	...
	/*if (pNetGame)
	{
		if (pNetGame->GetVehiclePool())
			pNetGame->GetVehiclePool()->ProcessPlatesTextures();
	}*/
...
void GameInstallHooks()
{
	...
	*(int*)0x53EB13 = dwGraphicsLoop - 0x53EB12 - 5; // relative addr
	...
	InstallHook(0x4D3AA0,(DWORD)AddAnimation_Hook,0x4D3934,
		AddAnimation_HookJmpCode, sizeof(AddAnimation_HookJmpCode));
	InstallHook(0x4D4610,(DWORD)BlendAnimation_Hook,0x4D4609,
		BlendAnimation_HookJmpCode, sizeof(BlendAnimation_HookJmpCode));
	...
	InstallCallHook(0x73C252, (DWORD)CameraShot_Hook, 0xE9);
}

...
static DWORD  dwPatchAddrModelInfo[] = {
...
static struct
{
	DWORD func_tbl;
	BYTE data[28];
} ModelInfoMemory[20000];

void RelocateModelInfoHack()
{
	DWORD oldProt;
	BYTE* aModelListMemory = (BYTE*)&ModelInfoMemory[0];

	int x=0;
	while (x!=20000)
	{
		ModelInfoMemory[x].func_tbl = 0x85BBF0;
		memset(ModelInfoMemory[x].data,0,28);
		x++;
	}
	...
	while(x!=14)
	{
		VirtualProtect((PVOID)dwPatchAddrModelInfo[x], 4, PAGE_EXECUTE_READWRITE, &oldProt);
		*(PDWORD)dwPatchAddrModelInfo[x] = (DWORD)aModelListMemory;
		x++;
	}
}

...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddEditBox( int ID, LPCTSTR strText, int x, int y, int width, int height, bool bIsDefault, CDXUTEditBox** ppCreated )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddIMEEditBox( int ID, LPCTSTR strText, int x, int y, int width, int height, bool bIsDefault, CDXUTIMEEditBox** ppCreated )
{
    HRESULT hr = S_OK;
...
void CDXUTDialog::InitDefaultElements()
{
    ...
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 1, D3DCOLOR_ARGB(255, 0, 0, 0), DT_LEFT | DT_TOP | DT_EXPANDTABS );
	    ...
	    case WM_CHAR:
        {
            switch( (TCHAR)wParam )
            {
                ...
                default:
                {
					...
					if(m_Buffer.GetTextSize() >= 128) return true;
                    ...
                    {
                        m_Buffer[m_nCaret] = (TCHAR)wParam;
                        PlaceCaret( m_nCaret + 1 );
                        m_nSelStart = m_nCaret;
                    } else
                    {
                        // Insert the char
                        if( m_Buffer.InsertChar( m_nCaret, (TCHAR)wParam ) )
                        {
                            PlaceCaret( m_nCaret + 1 );
                            m_nSelStart = m_nCaret;
                        }
...
TCHAR     CDXUTIMEEditBox::s_aszIndicator[5][3] = // String to draw to indicate current input locale
            {
                "En",
                "\x00",
                "\x00",
...
PCHAR    CDXUTIMEEditBox::s_wszCurrIndicator = CDXUTIMEEditBox::s_aszIndicator[0];  // Points to an indicator string that corresponds to current input locale
...
void CDXUTIMEEditBox::CheckInputLocale()
{
    ...
    {
        TCHAR wszLang[5];
        GetLocaleInfoA( MAKELCID( LOWORD( s_hklCurrent ), SORT_DEFAULT ), LOCALE_SABBREVLANGNAME, wszLang, 5 );
...
void CDXUTIMEEditBox::FinalizeString( bool bSend )
{
    ...
    {
        DXUTTRACE( "CDXUTIMEEditBox::FinalizeString: Reentrant detected!\n" );
        _ImmReleaseContext( DXUTGetHWND(), hImc );
        return;
    }
    bProcessing = true;
            ...
            if( wParam )
            {
                ...
                if( phKL )
                {
                    ...
                    for( UINT i = 0; i < cKL; ++i )
                    {
                            ...
                            case LANG_CHINESE:
                                switch( SUBLANGID( LOWORD( phKL[i] ) ) )
                                {
                                    ...
                                    case SUBLANG_CHINESE_TRADITIONAL:
                                        ...
                                        break;
                                    default:    // unsupported sub-language
                                        GetLocaleInfoA( MAKELCID( LOWORD( phKL[i] ), SORT_DEFAULT ), LOCALE_SABBREVLANGNAME, wszDesc, 128 );
                        ...
                        GetLocaleInfoA( MAKELCID( LOWORD( phKL[i] ), SORT_DEFAULT ), LOCALE_SLANGUAGE, wszDesc, 128 );
